...
def initialize_provider_result_data(
    provider_name, deserialized_model_tensors_descr, leaf_default_values
):
    provider_result = {
        "outputs": [],
        "data": {},
    ...
    for output_name, output_data in deserialized_model_tensors_descr.items():
        ...
        for field_to_check in ["shape", "element_type", "files"]:
            if field_to_check not in output_data.keys():
                provider_result["status"].append(
                    f'Deserialized tensors info of provider: "{provider_name}" doesn\'t'
                    f' contain important field "{field_to_check}": {output_data}.'
                    " Result is malformed"
                )
        ...
        # initialize data
        provider_result["data"][output_name] = initialize_provider_output_result_data(
            output_data, leaf_default_values
        )
...
def get_comparison_provider_output_result_data(
    ref_provider_name,
    ref_provider_output_data,
    provider_to_cmp_name,
    provider_to_cmp_output_data,
):
    for field_to_compare in ["shape", "element_type"]:
        if (
            ref_provider_output_data[field_to_compare]
            != provider_to_cmp_output_data[field_to_compare]
        ):
            provider_to_cmp_output_data["status"].append(
                f'Incorrect "{field_to_compare}":'
                f" {provider_to_cmp_output_data[field_to_compare]}, expected:"
                f" {ref_provider_output_data[field_to_compare]}"
            )
            ref_provider_output_data["status"].append(
                f'Incorrect "{field_to_compare}":'
                f" {ref_provider_output_data[field_to_compare]}, expected:"
                f" {provider_to_cmp_output_data[field_to_compare]}"
            )
    # If a reference output blob file is found in rhs provider,
    # we will conduct compasion of these files.
    ...
    # Ultimately, we compare only outputs/files that are coherent and
    # skip unmatching outputs/files
...
def get_comparison_provider_result_data(
    ref_provider_name,
    ref_provider_result_data,
    provider_to_cmp_name,
    provider_to_cmp_result_data,
):
    for ref_output_name in ref_provider_result_data["outputs"]:
        ...
        if ref_output_name in provider_to_cmp_result_data["data"].keys():
            provider_to_cmp_result_data["data"][ref_output_name] = (
                get_comparison_provider_output_result_data(
                    ref_provider_name,
                    ref_provider_result_data["data"][ref_output_name],
                    provider_to_cmp_name,
                    provider_to_cmp_result_data["data"][ref_output_name],
                )
            )
            ...
            if (
                "provider"
                not in provider_to_cmp_result_data["not_found_data"][ref_output_name].keys()
            ):
                provider_to_cmp_result_data["not_found_data"][ref_output_name]["provider"] = []
            provider_to_cmp_result_data["not_found_data"][ref_output_name]["provider"].append(
                ref_provider_name
            )
    ...
    for output_to_cmp_name in provider_to_cmp_result_data["outputs"]:
        if output_to_cmp_name not in ref_provider_result_data["data"].keys():
            if (
                "provider"
                not in ref_provider_result_data["not_found_data"][output_to_cmp_name].keys()
            ):
                ref_provider_result_data["not_found_data"][output_to_cmp_name]["provider"] = []
            ref_provider_result_data["not_found_data"][output_to_cmp_name]["provider"].append(
                provider_to_cmp_name
            )
...
def multi_provider_result_comparator(ref_provider, providers_to_compare, model_path, case_num):
    ...
    try:
        ...
        ref_provider_model_tensors_descr = printer.deserialize_output_tensor_descriptions(
            ref_provider_data_root_dir, model_name
        )
    ...
    for p in providers_to_compare:
        ...
        try:
            # read data from provider affiliated directories
            provider_model_tensors_descr = printer.deserialize_output_tensor_descriptions(
                provider_data_root_dir, model_name
            )
    ...
    if result["providers"][0] == ref_provider and len(result["providers"]) > 1:
        for p in result["providers"][1:]:
            result["data"][p] = get_comparison_provider_result_data(
                ref_provider, result["data"][ref_provider], p, result["data"][p]
            )
    return result
...
def main():
    parser = argparse.ArgumentParser(
        prog="multi-provider-blobs-comparator",
        description="""
...
each middele-layer nodes represents outputs of neural model, restored from output artifacts,
which are also compared with each other.
...
If these relevance conditions are not met for one or more model outputs,
they will be listed in a special leaf type called "not_found_data".
This allows to track inconsistency happens during model processing by different providers.
As necessary input arguments, it requires both a name of a reference provider and
names of providers to compare to that reference.""",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument("ref_provider", help="The name of a referecnce provider", default=None)
    parser.add_argument(
    ...
    parser.add_argument(
        "model", help="A model name or a path to a model contained file", default=None
    )
    args = parser.parse_args()

    case_num = int(0)
    return multi_provider_result_comparator(
        args.ref_provider, args.providers_to_compare, args.model, case_num
    )
